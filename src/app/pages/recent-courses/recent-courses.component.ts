        ...
        if (data) {
          ...
          this.restOfTheCourses.map((data) => {
            if (data.topic && data.topic.name) {
              data.name =
                data.topic.categories[0].name + ' - ' + data.topic.name;
            }
          });
          ...
          if (this.course1.topic && this.course1.topic.name) {
            this.course1.name =
              this.course1.topic.categories[0].name +
              ' - ' +
              this.course1.topic.name;
          }

          //Course 2
          if (this.course2.topic && this.course2.topic.name) {
            this.course2.name =
              this.course2.topic.categories[0].name +
              ' - ' +
              this.course2.topic.name;
          }

          //Course 3
          if (this.course3.topic && this.course3.topic.name) {
            this.course3.name =
              this.course3.topic.categories[0].name +
              ' - ' +
              this.course3.topic.name;
          }
        }
      });
    });
